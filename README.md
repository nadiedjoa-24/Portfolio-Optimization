...
| Notebook | Topic |
|---|---|
| [`trading_workshop_1_portfolio_optimization.ipynb`](trading_workshop_1_portfolio_optimization.ipynb) | **Modern Portfolio Theory**: expected return and risk, the covariance matrix, Monte Carlo sampling of the efficient frontier, and a backtest of a periodic-rebalancing strategy. |
...
| [`trading_workshop_3_risk_metrics.ipynb`](trading_workshop_3_risk_metrics.ipynb) | **Risk & performance metrics**: Sharpe and Sortino ratios, maximum drawdown, historical/parametric Value at Risk, and CAPM alpha/beta, implemented from their formulas. |

Every notebook fetches live market data through [`yfinance`](https://github.com/ranaroussi/yfinance) (or, for workshop 2, the intraday CSVs bundled in [`data/`](data/)) and has already been fully run, so you can read it straight on GitHub with nothing to execute. Numbers and charts will naturally differ if you re-run them later, since markets move.
...
Théophile Nadiedjoa ([@nadiedjoa-24](https://github.com/nadiedjoa-24)), Télécom Paris, 2026.
